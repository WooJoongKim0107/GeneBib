...
class ChiOne:
    ...
    def __post_init__(self):
        dbdt = get_dbdt()
        self.dbdt = np.array([dbdt[t] for t in range(1997, 2019)])
        self.sum = self.dbdt.sum()

    def __call__(self, si: Iterable) -> float:
        dbdt_est = dbdt_estimation(self.tau, self.beta, *si, start=1997, stop=2019)
...
class ChiTwo(ChiOne):
    ...
    def __post_init__(self):
        bt = get_bt()
        self.bt = np.array([bt[t] for t in range(1997, 2019)])
        self.sum = self.bt.sum()

    def __call__(self, c) -> float:
        bt_est = bt_estimation(self.tau, self.beta, *self.si, c, start=1997, stop=2019)
